...
def test(gitlab_api):
    ...
    response = requests.patch(API_ENDPOINT+'/jobs/'+expected_job.id+'/trace', test_log[:1000], headers=headers)
    # Check the response
    ...
    response = requests.patch(API_ENDPOINT+'/jobs/'+expected_job.id+'/trace', test_log[1000:1300], headers=headers)
    # Check the response
    assert response.status_code == 202
    ...
    response = requests.patch(API_ENDPOINT+'/jobs/'+expected_job.id+'/trace', test_log[1300:], headers=headers)
    # Check the response
    assert response.status_code == 202
...
def test_auth_error(gitlab_api):
    ...
    headers_to_try = {
        ...
        'Wrong token without content range': {
            'JOB-TOKEN': 'invalid_token',
        },
    }
    for name, headers in headers_to_try.items():
        response = requests.patch(API_ENDPOINT+'/jobs/'+expected_job.id+'/trace', test_log, headers=headers)
        # Check the response
