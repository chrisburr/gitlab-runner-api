...
class Job(object):
    ...
    def __init__(self, job_id, job_info, api):
        ...
        self._api._rsps.add_callback(
            responses.PUT, API_ENDPOINT+'/jobs/'+self.id,
            callback=self._update_job_callback,
        )
        # Append to the job's log
        self._api._rsps.add_callback(
            responses.PATCH, API_ENDPOINT+'/jobs/'+self.id+'/trace',
            callback=self._update_log_callback,
        )
        # Upload job artefacts
        self._api._rsps.add_callback(
            responses.POST, API_ENDPOINT+'/jobs/'+self.id+'/artifacts',
    ...
    def status(self, new_status):
        ...
        ordered_statuses = ['failed', 'pending', 'running', 'manual', 'canceled', 'success', 'skipped', 'created']
    ...
    def failure_reason(self, new_failure_reason):
        if self.status != 'failed':
            raise ValueError('Cannot set failure reason on a job with status: '+self.status)
    ...
    def _update_job_callback(self, request):
        ...
        if self.status != 'running':
            return (403, {}, json.dumps({'message': '403 Forbidden  - Job is not running'}))
        ...
        if 'state' in payload:
            if payload['state'] in ['success', 'failed']:
                ...
                print('API ignores this but an invalid job state was received:', payload['state'])

        return (200, {}, json.dumps(None))

    def _update_log_callback(self, request):
        if 'JOB-TOKEN' not in request.headers or request.headers['JOB-TOKEN'] != self.token:
    ...
    def upload_artifacts(self, filename, data):
        ...
        self._api._rsps.add_callback(
            responses.GET, API_ENDPOINT+'/jobs/'+self.id+'/artifacts',
    ...
    def _upload_artifacts_callback(self, request):
        ...
        if self.file_data is not None:
            return (400, {}, json.dumps({'message': '400 (Bad request) "Already uploaded" not given'}))

        payload = {}
        for part in decoder.MultipartDecoder(request.body, request.headers['Content-Type']).parts:
            header = part.headers[b'Content-Disposition'].decode(part.encoding).split('; ')
