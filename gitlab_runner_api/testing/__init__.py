...
class FakeGitlabAPI(object):
    ...
    def running_jobs(self):
        return [j for j in self._jobs
                if not isinstance(j, dict) and j.status == 'running']
    ...
    def __enter__(self):
        ...
        self._rsps.add_callback(
            responses.POST, API_ENDPOINT+'/runners/',
            callback=self._register_runner_callback
        )
        self._rsps.add_callback(
            responses.POST, API_ENDPOINT+'/runners/verify',
            callback=self._verify_runner_callback
        )
        self._rsps.add_callback(
            responses.POST, API_ENDPOINT+'/jobs/request',
        ...
        if self.n_with_artifacts > self.n_success + self.n_failed:
            raise ValueError('n_with_artifacts must be smaller than n_success + n_failed')
